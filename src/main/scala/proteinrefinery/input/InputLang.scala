...
import proteinrefinery.lib.{AgentIncreasesKinaseActivity, Binding, Protein, ProteinModifications, SiteLabel}

sealed trait InputLang[A]

object InputLang {
  case class A_Binds(bnd: Binding) extends InputLang[Unit]
  case class A_PhosTarget(kinase: Protein, substrate: Protein, site: SiteLabel) extends InputLang[Unit]
