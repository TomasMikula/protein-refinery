...
case class BindingPartnerPattern[Ref[_]](p: ProteinPattern[Ref], s: LocalSiteId[Ref]) {
  ...
  def bind(that: BindingPartnerPattern[Ref])(implicit ev: EqualK[Ref]): BindingData[Ref] = (for {
    i <- addAgent(this.p)
    j <- addAgent(that.p)
  ...
  } yield BindingData(Rule(lhs, List(a)), i, j, this.s, that.s)).eval(AgentsPattern.empty)
}
