...
class AutoUnificationMap[K, V] private[util](val entries: List[(K, V)]) extends AnyVal {
  ...
  def get[M[_]](k: K)(implicit I: Identification.Aux0[K, M], M: Monad[M]): M[List[V]] =
    entries.foldLeftM[M, List[V]](Nil)((vs, kv) => {
      val (k0, v) = kv
      I.unifyIfNecessary(k0, k).map({
        case Some(_) => v :: vs
        case None => vs
      })
    })
  ...
  def getEntries[M[_]](k: K)(implicit I: Identification.Aux0[K, M], M: Monad[M]): M[List[(K, V)]] =
  ...
  /** Returns the aggregate of the values whose keys must be unified with the given key,
    ...
    */
  def getUnifiedValue[M[_]](k: K)(f: (V, V) => M[V])(implicit I: Identification.Aux0[K, M], M: Monad[M]): M[Option[V]] =
    M.bind(get[M](k))({
  ...
  def getUnifiedEntry[M[_]](k: K)(f: (V, V) => M[V])(implicit I: Identification.Aux0[K, M], M: Monad[M]): M[Option[(K, V)]] =
    entries.foldLeftM[M, (K, Option[V])]((k, None))((acc, kv) => {
      val (k0, v0) = kv
      val (accK, accV) = acc
      I.unifyIfNecessary(k0, accK).flatMap[(K, Option[V])]({
  ...
  /** Add an entry to the map. The new key might be unified with some old keys, in which case
    ...
    */
  def put[M[_]](k: K, v: V)(f: (V, V) => M[V])(implicit I: Identification.Aux0[K, M], M: Monad[M]): M[AutoUnificationMap[K, V]] =
  ...
  def union[M[_]](that: AutoUnificationMap[K, V])(f: (V, V) => M[V])(implicit I: Identification.Aux0[K, M], M: Monad[M]): M[AutoUnificationMap[K, V]] =
  ...
  /** From both maps, retain only those entries whose key must be unified with some key from the other map.
    ...
    */
  def intersect[M[_]](that: AutoUnificationMap[K, V])(f: (V, V) => M[V])(implicit I: Identification.Aux0[K, M], M: Monad[M]): M[AutoUnificationMap[K, V]] =
  ...
  /** Accumulates every entry whose key must be unified with the given key. Also returns a list of untouched elements.
    ...
    */
  private def collect[M[_]](k: K, v: V)(f: (V, V) => M[V])(implicit I: Identification.Aux0[K, M], M: Monad[M]): M[(K, V, List[(K, V)])] =
    entries.foldLeftM[M, (K, V, List[(K, V)])]((k, v, Nil))((acc_untouched, kv) => {
      val (accK, accV, untouched) = acc_untouched
      val (k, v) = kv
      I.unifyIfNecessary(accK, k).flatMap({
