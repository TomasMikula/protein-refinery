...
class AutoUnificationMap[K, V] private[util](val entries: List[(K, V)]) extends AnyVal {
  ...
  def get[M[_]](k: K)(implicit U: Unification[M, K], M: Monad[M]): M[List[V]] =
  ...
  def getEntries[M[_]](k: K)(implicit U: Unification[M, K], M: Monad[M]): M[List[(K, V)]] =
  ...
  /** Returns the aggregate of the values whose keys must be unified with the given key,
    ...
    */
  def getUnifiedValue[M[_]](k: K)(f: (V, V) => M[V])(implicit U: Unification[M, K], M: Monad[M]): M[Option[V]] =
    M.bind(get[M](k))({
  ...
  def getUnifiedEntry[M[_]](k: K)(f: (V, V) => M[V])(implicit U: Unification[M, K], M: Monad[M]): M[Option[(K, V)]] =
    entries.foldLeftM[M, (K, Option[V])]((k, None))((acc, kv) => {
      val (k0, v0) = kv
      val (accK, accV) = acc
      U.mustUnify(k0, accK).flatMap({
  ...
  /** Add an entry to the map. The new key might be unified with some old keys, in which case
    ...
    */
  def put[M[_]](k: K, v: V)(f: (V, V) => M[V])(implicit U: Unification[M, K], M: Monad[M]): M[AutoUnificationMap[K, V]] =
  ...
  def union[M[_]](that: AutoUnificationMap[K, V])(f: (V, V) => M[V])(implicit U: Unification[M, K], M: Monad[M]): M[AutoUnificationMap[K, V]] =
  ...
  /** From both maps, retain only those entries whose key must be unified with some key from the other map.
    ...
    */
  def intersect[M[_]](that: AutoUnificationMap[K, V])(f: (V, V) => M[V])(implicit U: Unification[M, K], M: Monad[M]): M[AutoUnificationMap[K, V]] =
  ...
  /** Accumulates every entry whose key must be unified with the given key. Also returns a list of untouched elements.
    ...
    */
  private def collect[M[_]](k: K, v: V)(f: (V, V) => M[V])(implicit U: Unification[M, K], M: Monad[M]): M[(K, V, List[(K, V)])] =
